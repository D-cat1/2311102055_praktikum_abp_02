<div class="space-y-6">
    <div>
        ...
        <flux:text class="mt-1">{{ __('Tambahkan produk baru ke inventory.') }}</flux:text>
    </div>
        ...
        <flux:field>
            ...
            @if ($image)
                <div class="flex items-center gap-4 rounded-lg border border-zinc-200 p-3 dark:border-zinc-700">
                    <img src="{{ $image->temporaryUrl() }}" alt="Preview" class="h-16 w-16 rounded-lg object-cover" />
                    <div class="flex-1">
                        <flux:text variant="strong">{{ $image->getClientOriginalName() }}</flux:text>
                        <flux:text class="text-xs">{{ number_format($image->getSize() / 1024, 1) }} KB</flux:text>
                    </div>
                    <flux:button variant="ghost" size="sm" icon="x-mark" wire:click="removeImage" />
                </div>
            @else
                <label class="flex cursor-pointer flex-col items-center gap-2 rounded-lg border-2 border-dashed border-zinc-300 px-6 py-8 transition hover:border-zinc-400 dark:border-zinc-600 dark:hover:border-zinc-500">
                    ...
                    <flux:text class="text-sm">{{ __('Klik untuk upload gambar') }}</flux:text>
